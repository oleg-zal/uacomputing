...
if ( ! version_compare( get_bloginfo( 'version' ), '5.2', '>=' ) && ! function_exists( 'sodium_crypto_box' ) ) {
	require_once dirname( EasyWPSMTP_PLUGIN_FILE ) . '/libs/sodium_compat/autoload.php';
}
...
class Crypto {
	...
	public static function get_secret_key( $create = false ) {
		...
		if ( $create ) {
			...
			add_option( 'easy_wp_smtp_mail_key', base64_encode( $secret_key ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
