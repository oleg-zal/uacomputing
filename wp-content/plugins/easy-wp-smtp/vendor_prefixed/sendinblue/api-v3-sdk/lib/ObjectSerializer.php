 ...
 */
namespace EasyWPSMTP\Vendor\SendinBlue\Client;
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        if (null === $data) {
            ...
            if (\array_key_exists('Content-Disposition', $httpHeaders) && \preg_match('/inline; filename=[\'"]?([^\'"\\s]+)[\'"]?$/i', $httpHeaders['Content-Disposition'], $match)) {
                $filename = \EasyWPSMTP\Vendor\SendinBlue\Client\Configuration::getDefaultConfiguration()->getTempFolderPath() . \DIRECTORY_SEPARATOR . self::sanitizeFilename($match[1]);
            } else {
                $filename = \tempnam(\EasyWPSMTP\Vendor\SendinBlue\Client\Configuration::getDefaultConfiguration()->getTempFolderPath(), '');
            }
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && \is_string($data->{$discriminator})) {
                $subclass = 'EasyWPSMTP\\Vendor\\SendinBlue\\Client\\Model\\' . $data->{$discriminator};
