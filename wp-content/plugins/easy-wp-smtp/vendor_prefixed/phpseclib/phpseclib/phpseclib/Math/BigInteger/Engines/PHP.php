 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class PHP extends \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\Engine
{
    /**#@+
     * Array constants
     *
    ...
    public function __construct($x = 0, $base = 10)
    {
        ...
        if (!static::$isValidEngine[static::class]) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\BadConfigurationException(static::class . ' is not setup correctly on this system');
        }
        $this->value = [];
    ...
    protected function initialize($base)
    {
        switch (\abs($base)) {
            case 16:
                ...
                $temp = new static(\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::hex2bin($x), 256);
     ...
     */
    protected function divideHelper(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $y)
    {
        if (\count($y->value) == 1) {
     ...
     * @return static
     */
    protected function normalize(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $result)
    {
     ...
     * @return PHP
     */
    protected function powModInner(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $e, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $n)
    {
        try {
            $class = static::$modexpEngine[static::class];
            return $class::powModHelper($this, $e, $n, static::class);
        } catch (\Exception $err) {
            return \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP\DefaultEngine::powModHelper($this, $e, $n, static::class);
        }
    }
    /**
     * Performs squaring
     ...
     * @see self::isPrime()
     */
    public static function scan1divide(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $r)
    {
        $r_value =& $r->value;
     ...
     * @return PHP
     */
    protected function powHelper(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP $n)
    {
