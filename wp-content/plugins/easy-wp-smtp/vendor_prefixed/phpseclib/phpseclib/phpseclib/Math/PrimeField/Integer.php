 ...
 */
namespace EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField;

use EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings;
use EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger;
use EasyWPSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer as Base;
/**
 * Prime Finite Fields
 *
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Integer extends \EasyWPSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer
{
    /**
     ...
     */
    public function __construct($instanceID, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $num = null)
    {
     ...
     * @return void
     */
    public static function setModulo($instanceID, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $modulo)
    {
    ...
    public static function setRecurringModuloFunction($instanceID, callable $function)
    {
        ...
        if (!isset(static::$zero[static::class])) {
            static::$zero[static::class] = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger();
        }
    }
    /**
     ...
     * @return static
     */
    public function pow(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $x)
    {
        $temp = new static($this->instanceID);
    ...
    public function squareRoot()
    {
        ...
        if (!isset($one)) {
            ...
            $two = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(2);
        }
        $reduce = static::$reduce[$this->instanceID];
        $p_1 = static::$modulo[$this->instanceID]->subtract($one);
        $q = clone $p_1;
        $s = \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger::scan1divide($q);
        ...
        $m = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($s);
    ...
    {
        return \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($this->toBytes());
    }
    /**
    ...
    {
        $w++;
        $mask = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger((1 << $w) - 1);
        $sub = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(1 << $w);
