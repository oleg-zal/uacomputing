...
use EasyWPSMTP\Vendor\phpseclib3\Crypt\DSA;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA;
use EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException;
...
abstract class AsymmetricKey
{
    ...
    protected function __construct()
    {
        ...
        $this->hash = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash('sha256');
        $this->hmac = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash('sha256');
    }
    /**
    ...
    {
        if (!isset(self::$zero)) {
            self::$zero = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(0);
            self::$one = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(1);
        }
    ...
    public static function load($key, $password = \false)
    {
        ...
        if ($components === \false) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException('Unable to read key');
        }
        ...
        $new->format = $format;
        $new->comment = $comment;
        return $new instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PrivateKey ? $new->withPassword($password) : $new;
    }
    /**
     * Loads a private key
     *
    ...
    public static function loadPrivateKey($key, $password = '')
    {
        ...
        if (!$key instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PrivateKey) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a private key');
        }
        return $key;
    }
    /**
    ...
    {
        $key = self::load($key);
        if (!$key instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a public key');
        }
        return $key;
    }
    /**
    ...
    {
        $key = self::load($key);
        if (!$key instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PrivateKey && !$key instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException('The key that was loaded was not a parameter');
        }
        return $key;
    }
    /**
    ...
    protected static function validatePlugin($format, $type, $method = null)
    {
        ...
        if (!isset(self::$plugins[static::ALGORITHM][$format][$type])) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedFormatException("{$type} is not a supported format");
        }
        $type = self::$plugins[static::ALGORITHM][$format][$type];
        if (isset($method) && !\method_exists($type, $method)) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedFormatException("{$type} does not implement {$method}");
        }
        return $type;
    }
    /**
    ...
    private static function loadPlugins($format)
    {
        if (!isset(self::$plugins[static::ALGORITHM][$format])) {
            ...
            foreach (new \DirectoryIterator(__DIR__ . '/../' . static::ALGORITHM . '/Formats/' . $format . '/') as $file) {
                ...
                $type = 'EasyWPSMTP\\Vendor\\phpseclib3\\Crypt\\' . static::ALGORITHM . '\\Formats\\' . $format . '\\' . $name;
    ...
    public function getLoadedFormat()
    {
        if (empty($this->format)) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\NoKeyLoadedException('This key was created with createKey - it was not loaded with load. Therefore there is no "loaded format"');
        }
    ...
    {
        $new = clone $this;
        $new->hash = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash($hash);
        $new->hmac = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash($hash);
        return $new;
    }
    /**
    ...
    {
        $v = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($in, 256);
        $vlen = \strlen($in) << 3;
