...
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime as PrimeCurve;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards as TwistedEdwardsCurve;
use EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException;
...
trait Common
{
     ...
     */
    public static function setImplicitCurve(\EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\\EasyWPSMTP\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $params['namedCurve'];
            if (!\class_exists($curve)) {
                throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $params['namedCurve'] . ' is not supported');
            }
            return new $curve();
        }
        ...
        if (isset($params['specifiedCurve'])) {
            ...
            switch ($data['fieldID']['fieldType']) {
                case 'prime-field':
                    $curve = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime();
                    $curve->setModulo($data['fieldID']['parameters']);
                    $curve->setCoefficients(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($data['curve']['a'], 256), new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($data['curve']['b'], 256));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                case 'characteristic-two-field':
                    $curve = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary();
                    $params = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($data['fieldID']['parameters']);
                    $params = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($params[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
                    ...
                    switch ($params['basis']) {
                        case 'tpBasis':
                            ...
                            break;
                        case 'ppBasis':
                            $temp = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($params['parameters']);
                            $temp = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($temp[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    ...
                    $len = \ceil($modulo[0] / 8);
                    $curve->setCoefficients(\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($data['curve']['a']), \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($data['curve']['b']));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                default:
                    throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $data['fieldID']['fieldType'] . ' is not supported');
            }
        }
     ...
     * @return object[]
     */
    public static function extractPoint($str, \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            ...
            $y[0] = $y[0] & \chr(0x7f);
            $y = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($y, 256);
        ...
        if (($val = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::shift($str)) != "\0") {
            throw new \UnexpectedValueException('extractPoint expects the first byte to be null - not ' . \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($val));
        }
        if ($str == "\0") {
            return [];
        }
        ...
        if ($keylen == 2 * $order + 1) {
            ...
            list(, $w, $x, $y) = $matches;
            if ($w != "\4") {
                throw new \UnexpectedValueException('The first byte of an uncompressed point should be 04 - not ' . \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($val));
            }
            $point = [$curve->convertInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($x, 256)), $curve->convertInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($y, 256))];
            if (!$curve->verifyPoint($point)) {
                throw new \RuntimeException('Unable to verify that point exists on curve');
            }
            return $point;
     ...
     * @return string|false
     */
    private static function encodeParameters(\EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $returnArray = \false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            if (isset(self::$curveOIDs[$name])) {
                ...
                return $returnArray ? ['namedCurve' => $name] : \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $name], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
            }
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'EasyWPSMTP\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $testName;
                ...
                switch ($name) {
                    case 'Prime':
                        if (!$candidate instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime) {
                            break;
                        }
                        ...
                        return $returnArray ? ['namedCurve' => $testName] : \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $testName], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
                    case 'Binary':
                        if (!$candidate instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary) {
                            break;
                        }
        ...
        $x = $point[0]->toBytes();
        $y = $point[1]->toBytes();
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime) {
            /*
             * valid versions are:
             *
             * ecdpVer1:
            ...
            return $returnArray ? ['specifiedCurve' => $data] : \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER(['specifiedCurve' => $data], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        }
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary) {
            ...
            switch ($basis) {
                case 3:
                    $basis = 'tpBasis';
                    $modulo = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($modulo[0]);
                    break;
                case 5:
                    $basis = 'ppBasis';
                    // these should be in strictly ascending order (hence the commented out rsort above)
                    $modulo = ['k1' => new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($modulo[2]), 'k2' => new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($modulo[1]), 'k3' => new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($modulo[0])];
                    $modulo = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER($modulo, \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    $modulo = new \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Element($modulo);
            }
            $params = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER(['m' => new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($m), 'basis' => $basis, 'parameters' => $modulo], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
        ...
        throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Curve cannot be serialized');
    }
    /**
     * Use Specified Curve
     *
