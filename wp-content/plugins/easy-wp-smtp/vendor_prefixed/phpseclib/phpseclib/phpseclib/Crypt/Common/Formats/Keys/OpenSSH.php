...
abstract class OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (\strpos($key, 'BEGIN OPENSSH PRIVATE KEY') !== \false) {
            ...
            $magic = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::shift($key, 15);
            if ($magic != "openssh-key-v1\0") {
                throw new \RuntimeException('Expected openssh-key-v1');
            }
            list($ciphername, $kdfname, $kdfoptions, $numKeys) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('sssN', $key);
            if ($numKeys != 1) {
            ...
            switch ($ciphername) {
                ...
                case 'aes256-ctr':
                    ...
                    list($salt, $rounds) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('sN', $kdfoptions);
                    $crypto = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\AES('ctr');
            ...
            list($publicKey, $paddedKey) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $key);
            list($type) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $publicKey);
            if (isset($crypto)) {
                $paddedKey = $crypto->decrypt($paddedKey);
            }
            list($checkint1, $checkint2) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('NN', $paddedKey);
        ...
        list($type) = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $key);
        self::checkType($type);
    ...
    {
        list(, $checkint) = \unpack('N', \EasyWPSMTP\Vendor\phpseclib3\Crypt\Random::string(4));
        $comment = isset($options['comment']) ? $options['comment'] : self::$comment;
        $paddedKey = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('NN', $checkint, $checkint) . $privateKey . \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('s', $comment);
        ...
        */
        ...
        if (!$usesEncryption) {
            $key = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('sssNss', 'none', 'none', '', 1, $publicKey, $paddedKey);
        } else {
            $rounds = isset($options['rounds']) ? $options['rounds'] : 16;
            $salt = \EasyWPSMTP\Vendor\phpseclib3\Crypt\Random::string(16);
            $kdfoptions = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('sN', $salt, $rounds);
            $crypto = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\AES('ctr');
            $crypto->setPassword($password, 'bcrypt', $salt, $rounds, 32);
            $paddedKey = $crypto->encrypt($paddedKey);
            $key = \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('sssNss', 'aes256-ctr', 'bcrypt', $kdfoptions, 1, $publicKey, $paddedKey);
        }
        $key = "openssh-key-v1\0{$key}";
        return "-----BEGIN OPENSSH PRIVATE KEY-----\n" . \chunk_split(\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($key), 70, "\n") . "-----END OPENSSH PRIVATE KEY-----\n";
    }
}
