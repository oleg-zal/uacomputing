 ...
 */
namespace EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA;
...
use EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo;
...
final class PublicKey extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA implements \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey
{
     ...
     */
    private function exponentiate(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $x)
    {
    ...
    private function rsassa_pkcs1_v1_5_verify($m, $s)
    {
        ...
        try {
            ...
            $exception = \true;
        } catch (\EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException $e) {
            $r2 = \false;
        }
        if ($exception) {
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        if (\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::shift($em, 2) != "\0\1") {
            return \false;
        }
        $em = \ltrim($em, "ÿ");
        if (\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::shift($em) != "\0") {
            return \false;
        }
        $decoded = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($em);
        ...
        if (!isset($oids)) {
            ...
            \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::loadOIDs($oids);
        }
        $decoded = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo::MAP);
        ...
        $hash = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash($hash);
        $em = $hash->hash($m);
        $em2 = $decoded['digest'];
    ...
    private function rsaes_pkcs1_v1_5_encrypt($m, $pkcs15_compat = \false)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \EasyWPSMTP\Vendor\phpseclib3\Crypt\Random::string($psLen - \strlen($ps));
    ...
    private function rsaes_oaep_encrypt($m)
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \EasyWPSMTP\Vendor\phpseclib3\Crypt\Random::string($this->hLen);
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                ...
                throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedFormatException('The PSS format can only be used when the signature method has been explicitly set to PSS');
            }
        }
    ...
    {
        $new = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
