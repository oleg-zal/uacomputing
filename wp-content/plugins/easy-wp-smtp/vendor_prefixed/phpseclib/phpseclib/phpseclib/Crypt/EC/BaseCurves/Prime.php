 ...
 */
namespace EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves;

use EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings;
use EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger;
use EasyWPSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer;
use EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField;
use EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField\Integer as PrimeInteger;
/**
 * Curves over y^2 = x^3 + a*x + b
 *
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Prime extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base
{
    /**
     ...
     * Sets the modulo
     */
    public function setModulo(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $modulo)
    {
        $this->modulo = $modulo;
        $this->factory = new \EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField($modulo);
        $this->two = $this->factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(2));
        $this->three = $this->factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(3));
        // used by jacobian coordinates
        $this->one = $this->factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(1));
        $this->four = $this->factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(4));
        $this->eight = $this->factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(8));
    }
    /**
     * Set coefficients a and b
     */
    public function setCoefficients(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $a, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $b)
    {
    ...
    {
        switch (\true) {
            case !$x instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger && !$x instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField\Integer:
                throw new \UnexpectedValueException('EasyWPSMTP\\Vendor\\Argument 1 passed to Prime::setBasePoint() must be an instance of either BigInteger or PrimeField\\Integer');
            case !$y instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger && !$y instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField\Integer:
                throw new \UnexpectedValueException('EasyWPSMTP\\Vendor\\Argument 2 passed to Prime::setBasePoint() must be an instance of either BigInteger or PrimeField\\Integer');
        }
        if (!isset($this->factory)) {
            throw new \RuntimeException('setModulo needs to be called before this method');
        }
        $this->p = [$x instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger ? $this->factory->newInteger($x) : $x, $y instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger ? $this->factory->newInteger($y) : $y];
    }
    /**
    ...
    {
        $y = \ord(\EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::shift($m));
        $x = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($m, 256);
     ...
     * @return int[]
     */
    private static function getJSFPoints(\EasyWPSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer $k1, \EasyWPSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer $k2)
    {
        static $three;
        if (!isset($three)) {
            $three = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(3);
        }
        $jsf = [[], []];
        $k1 = $k1->toBigInteger();
        $k2 = $k2->toBigInteger();
        $d1 = 0;
        $d2 = 0;
        while ($k1->compare(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(-$d1)) > 0 || $k2->compare(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(-$d2)) > 0) {
            // first phase
