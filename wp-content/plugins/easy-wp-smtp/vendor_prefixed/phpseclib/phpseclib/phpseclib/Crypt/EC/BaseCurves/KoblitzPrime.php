 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class KoblitzPrime extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime
{
    /**
     * Basis
     *
    ...
    public function multiplyAddPoints(array $points, array $scalars)
    {
        ...
        if (!isset($this->basis)) {
            $factory = new \EasyWPSMTP\Vendor\phpseclib3\Math\PrimeField($this->order);
            $tempOne = $factory->newInteger($one);
            $tempTwo = $factory->newInteger($two);
            $tempThree = $factory->newInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(3));
     ...
     * @return BigInteger[]
     */
    protected static function extendedGCD(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $u, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $v)
    {
        $one = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(1);
        $zero = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger();
        $a = clone $one;
        $b = clone $zero;
        $c = clone $zero;
        $d = clone $one;
