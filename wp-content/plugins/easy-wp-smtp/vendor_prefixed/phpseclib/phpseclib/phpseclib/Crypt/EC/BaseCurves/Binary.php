...
use EasyWPSMTP\Vendor\phpseclib3\Math\BinaryField\Integer as BinaryInteger;
/**
 * Curves over y^2 + x*y = x^3 + a*x^2 + b
 *
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Binary extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base
{
    /**
    ...
    {
        $this->modulo = $modulo;
        $this->factory = new \EasyWPSMTP\Vendor\phpseclib3\Math\BinaryField(...$modulo);
    ...
    {
        switch (\true) {
            case !\is_string($x) && !$x instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BinaryField\Integer:
                throw new \UnexpectedValueException('EasyWPSMTP\\Vendor\\Argument 1 passed to Binary::setBasePoint() must be a string or an instance of BinaryField\\Integer');
            case !\is_string($y) && !$y instanceof \EasyWPSMTP\Vendor\phpseclib3\Math\BinaryField\Integer:
                throw new \UnexpectedValueException('EasyWPSMTP\\Vendor\\Argument 2 passed to Binary::setBasePoint() must be a string or an instance of BinaryField\\Integer');
        }
        if (!isset($this->factory)) {
            throw new \RuntimeException('setModulo needs to be called before this method');
        }
