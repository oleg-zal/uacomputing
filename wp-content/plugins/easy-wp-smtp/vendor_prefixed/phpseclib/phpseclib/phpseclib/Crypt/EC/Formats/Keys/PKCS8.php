...
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed448;
...
abstract class PKCS8 extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $params = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        if (!$params) {
            throw new \RuntimeException('EasyWPSMTP\\Vendor\\Unable to decode the parameters using Maps\\ECParameters');
        }
        $components = [];
        ...
        $decoded = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($key['privateKey']);
        if (!$decoded) {
            throw new \RuntimeException('Unable to decode BER');
        }
    ...
    private static function loadEdDSA(array $key)
    {
        ...
        if (isset($key['privateKey'])) {
            $components['curve'] = $key['privateKeyAlgorithm']['algorithm'] == 'id-Ed25519' ? new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519() : new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed448();
            // 0x04 == octet string
        ...
        if (isset($key['publicKey'])) {
            if (!isset($components['curve'])) {
                $components['curve'] = $key['publicKeyAlgorithm']['algorithm'] == 'id-Ed25519' ? new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519() : new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed448();
            }
    ...
    {
        self::initialize_static_variables();
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException('Montgomery Curves are not supported');
        }
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            return self::wrapPublicKey($curve->encodePoint($publicKey), null, $curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519 ? 'id-Ed25519' : 'id-Ed448');
        }
        $params = new \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Element(self::encodeParameters($curve, \false, $options));
    ...
    public static function savePrivateKey(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $privateKey, \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $secret = null, $password = '', array $options = [])
    {
        ...
        if ($curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            return self::wrapPrivateKey("\4 " . $secret, [], null, $password, $curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519 ? 'id-Ed25519' : 'id-Ed448');
        }
