...
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve448;
...
abstract class MontgomeryPrivate
{
    ...
    public static function load($key, $password = '')
    {
        switch (\strlen($key)) {
            case 32:
                $curve = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve25519();
                break;
            case 56:
                $curve = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve448();
                break;
            default:
                throw new \LengthException('The only supported lengths are 32 and 56');
        }
        $components = ['curve' => $curve];
        $components['dA'] = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($key, 256);
     ...
     * @return string
     */
    public static function savePublicKey(\EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery $curve, array $publicKey)
    {
     ...
     * @return string
     */
    public static function savePrivateKey(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $privateKey, \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery $curve, array $publicKey, $secret = null, $password = '')
    {
        if (!empty($password) && \is_string($password)) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedFormatException('MontgomeryPrivate private keys do not support encryption');
        }
