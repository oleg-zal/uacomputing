 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class PSS extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    /**
     * OID Name
     *
    ...
    public static function load($key, $password = '')
    {
        ...
        $result = $components + \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::load($key[$type . 'Key']);
        if (isset($key[$type . 'KeyAlgorithm']['parameters'])) {
            $decoded = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($key[$type . 'KeyAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP);
        } else {
            $params = [];
        }
        if (isset($params['maskGenAlgorithm']['parameters'])) {
            $decoded = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($params['maskGenAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params['maskGenAlgorithm']['parameters'] = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
        } else {
    ...
    {
        self::initialize_static_variables();
        $key = \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::savePrivateKey($n, $e, $d, $primes, $exponents, $coefficients);
        $key = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::extractBER($key);
     ...
     * @return string
     */
    public static function savePublicKey(\EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $n, \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger $e, array $options = [])
    {
        self::initialize_static_variables();
        $key = \EasyWPSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PKCS1::savePublicKey($n, $e);
        $key = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::extractBER($key);
        $params = self::savePSSParams($options);
    ...
    public static function savePSSParams(array $options)
    {
        ...
        */
        $params = ['trailerField' => new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(1)];
        if (isset($options['hash'])) {
            $params['hashAlgorithm']['algorithm'] = 'id-' . $options['hash'];
        }
        if (isset($options['MGFHash'])) {
            $temp = ['algorithm' => 'id-' . $options['MGFHash']];
            $temp = \EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER($temp, \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
            $params['maskGenAlgorithm'] = ['algorithm' => 'id-mgf1', 'parameters' => new \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Element($temp)];
        }
        if (isset($options['saltLength'])) {
            $params['saltLength'] = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($options['saltLength']);
        }
        return new \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Element(\EasyWPSMTP\Vendor\phpseclib3\File\ASN1::encodeDER($params, \EasyWPSMTP\Vendor\phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP));
    }
}
