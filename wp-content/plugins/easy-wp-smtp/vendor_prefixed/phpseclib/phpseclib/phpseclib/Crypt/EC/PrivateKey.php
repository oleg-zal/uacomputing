 ...
 */
namespace EasyWPSMTP\Vendor\phpseclib3\Crypt\EC;

use EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\Common;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC;
...
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve25519;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Keys\PKCS1;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Signature\ASN1 as ASN1Signature;
use EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash;
use EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedOperationException;
...
final class PrivateKey extends \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC implements \EasyWPSMTP\Vendor\phpseclib3\Crypt\Common\PrivateKey
{
    ...
    {
        if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Curve25519 && self::$engines['libsodium']) {
                return \sodium_crypto_scalarmult($this->dA->toBytes(), $coordinates);
            }
            $point = [$this->curve->convertInteger(new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger(\strrev($coordinates), 256))];
            $point = $this->curve->multiplyPoint($point, $this->dA);
            return \strrev($point[0]->toBytes(\true));
        }
        if (!$this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            $coordinates = "\0{$coordinates}";
        }
        $point = \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint($coordinates, $this->curve);
        $point = $this->curve->multiplyPoint($point, $this->dA);
        if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
    ...
    {
        if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            throw new \EasyWPSMTP\Vendor\phpseclib3\Exception\UnsupportedOperationException('Montgomery Curves cannot be used to create signatures');
        }
        $dA = $this->dA;
        ...
        if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Curves\Ed25519 && self::$engines['libsodium'] && !isset($this->context)) {
                $result = \sodium_crypto_sign_detached($message, $this->withPassword()->toString('libsodium'));
                return $shortFormat == 'SSH2' ? \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'ssh-' . \strtolower($this->getCurve()), $result) : $result;
            }
            ...
            $curve = $this->curve;
            $hash = new \EasyWPSMTP\Vendor\phpseclib3\Crypt\Hash($curve::HASH);
            ...
            $r = \strrev($r);
            $r = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($r, 256);
            list(, $r) = $r->divide($order);
            $R = $curve->multiplyPoint($curve->getBasePoint(), $r);
            $R = $curve->encodePoint($R);
            $k = $hash->hash($dom . $R . $A . $message);
            $k = \strrev($k);
            $k = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($k, 256);
            list(, $k) = $k->divide($order);
            $S = $k->multiply($dA)->add($r);
            list(, $S) = $S->divide($order);
            $S = \str_pad(\strrev($S->toBytes()), $curve::SIZE, "\0");
            return $shortFormat == 'SSH2' ? \EasyWPSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('ss', 'ssh-' . \strtolower($this->getCurve()), $R . $S) : $R . $S;
        }
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            ...
            if ($result) {
                ...
                \extract(\EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Signature\ASN1::load($signature));
                return $shortFormat == 'SSH2' ? $format::save($r, $s, $this->getCurve()) : $format::save($r, $s);
            }
        }
        $e = $this->hash->hash($message);
        $e = new \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger($e, 256);
        $Ln = $this->hash->getLength() - $order->getLength();
        $z = $Ln > 0 ? $e->bitwise_rightShift($Ln) : $e;
        while (\true) {
            $k = \EasyWPSMTP\Vendor\phpseclib3\Math\BigInteger::randomRange(self::$one, $order->subtract(self::$one));
    ...
    public function getPublicKey()
    {
        ...
        $key = \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC::loadFormat($format, $key);
        if ($this->curve instanceof \EasyWPSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            return $key;
        }
